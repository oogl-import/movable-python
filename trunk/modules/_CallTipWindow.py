...
def bindButtonImages(button, enterIMG, leaveIMG, text):
    callTip = CallTip(button)
    def enter(event):
        button.configure(image=enterIMG)
        callTip.showtip(text)
    def leave(event):
        button.configure(image=leaveIMG)
        callTip.hidetip()
    button.bind('<Enter>', enter)
    button.bind('<Leave>', leave)


def createCallTip(widget, text):
    callTip = CallTip(widget)
    def enter(event):
        callTip.showtip(text)
    def leave(event):
        callTip.hidetip()
